...
> **Status 2026-07-31 (end of day):** #1 and #2 implemented, flashed, and
> confirmed working. KC_CCCV on Fn+C (tap = paste, hold = copy). The heatmap
> renders white heat over the configured red base and is the power-on default;
> Fn+Up wraps to plain Solid Color, Fn+Down returns. #3 deferred — the numpad
> layer may become a physical GMMK Numpad instead. Remember the VIA gotcha:
> key changes need the bootmagic-reset flash (see build_and_flash.md).
